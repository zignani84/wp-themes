...
add_action( 'admin_menu', 'motivos_banner_remove_meta_box');
function motivos_banner_remove_meta_box(){
   remove_meta_box('motivos_bannerdiv', 'banner', 'normal');
}
//Add new taxonomy meta box
add_action( 'add_meta_boxes', 'motivos_banner_add_meta_box');
function motivos_banner_add_meta_box() {
	add_meta_box( 'motivos_banner', 'Motivos','motivos_banner_metabox','banner' ,'side','core');
}
//Callback to set up the metabox
function motivos_banner_metabox( $post ) {
    //Get taxonomy and terms
    $taxonomy = 'motivos_banner';
...
add_action('admin_enqueue_scripts','motivos_banner_radiotax_javascript');
function motivos_banner_radiotax_javascript(){
...
function banner_post_type() {
	...
	register_taxonomy( 'motivos_banner',
		array( 'banner' ),
		array(
			'hierarchical'      => true,
			'show_ui'           => true,
...
function add_home_banner_fields ()
{
  function fun_slider_banner()
  {
    ...
    $html .= '</td>';
    $html .= '<td style="">';
    $html .= '<input type="text" class="attachmentlinks" style="width: 100%;" name="banner_link[href]" value="' . (isset($banner_link['href']) ? $banner_link['href'] : '') . '" />';
    $html .= '</td>';
    $html .= '</tr>';
    $html .= '</table>';
    echo $html;
...
function save_home_banner_custom_meta_data($id = null)
{
  if (!current_user_can('edit_page', $id))
    ...
    if (isset($_POST['banner_link'])) {
      update_post_meta($id, 'banner_link', $_POST['banner_link']);
    }
}
