...
function destino_cpt_metaboxCallback() {
	...
	//ID EPS
	add_meta_box('destino_idEps', __('ID EPS'),'destino_metaboxIdEps','destino','normal');
}
...
function save_destino_metadata($post_id){
	...
	if( isset($_POST["destino_dados"]) && is_array($_POST["destino_dados"]) && isset($_POST["destino_fraseDestaque"]) && isset($_POST["destino_textoPrincipal"]) ) {
		...
		//ID EPS
		update_post_meta($post_id,'destino_idEps',$_POST["destino_idEps"]);
	}
			...
			<div class="col-md-12">
				<div id="containerBannerPrincipal" class="row" data-maxitens="1">
					...
					if(isset($destino_dados["bannerPrincipal"]) && !empty($destino_dados["bannerPrincipal"])) {
						$destino_dados["bannerPrincipal"] = intval($destino_dados["bannerPrincipal"]);
						$imgUrl = get_relative_thumb($destino_dados["bannerPrincipal"],'thumbnail');
						?>
			...
			<div class="col-md-12">
				<div id="containerImagemDestaque" class="row" data-maxitens="1">
					...
					if(isset($destino_dados["imagemDestaque"]) && !empty($destino_dados["imagemDestaque"])) {
						$destino_dados["imagemDestaque"] = intval($destino_dados["imagemDestaque"]);
						$imgUrl = get_relative_thumb($destino_dados["imagemDestaque"],'thumbnail');
						?>
						<div class="col-md-3 itemMidia">
							<div class="mediaThumb" style="background-image:url(<?php echo $imgUrl;?>);">
								<span class="dashicons dashicons-no btRemover"></span>
		...
		<div class="row">
			...
			<div class="col-md-3">
				<div class="form-group">
					...
					<input class="form-control" type="text" name="destino_dados[idioma]" <?php echo 'value="'.$destino_dados["idioma"].'"'; ?>>
				</div>
			</div>

			<div class="col-md-3">
				<div class="form-group">
					<label><?php _e('Capital do País')?></label>
					<input class="form-control" type="text" name="destino_dados[capital]" <?php echo 'value="'.$destino_dados["capital"].'"'; ?>>
				</div>
			</div>
...
function destino_metaboxGuiaVivencia() {
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-md-3">
				<div class="form-group">
					<label><?php _e('Ticket Avulso de Onibus')?></label>
			...
			<div class="col-md-3">
				<div class="form-group">
					<label><?php _e('Café da Manhã')?></label>
							...
							<div class="col-md-12">
								<div id="containerImagemAtracao_<?php echo $i; ?>" class="row" data-maxitens="1">
									...
									if(isset($atracao["imagem"]) && !empty($atracao["imagem"])) {
										$atracao["imagem"] = intval($atracao["imagem"]);
										$imgUrl = get_relative_thumb($atracao["imagem"],'thumbnail');
										?>
										<div class="col-md-3 itemMidia">
											<div class="mediaThumb" style="background-image:url(<?php echo $imgUrl;?>);">
												<span class="dashicons dashicons-no btRemover"></span>
...
// 9. ID EPS
function destino_metaboxIdEps() {
	global $post;

	$destino_idEps = get_post_meta($post->ID,'destino_idEps',true);
	?>
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-md-12">
				<div class="form-group">
					<label><?php _e('Preencher com o ID correspondente ao destino no EPS.')?></label>
					<input class="form-control" type="text" name="destino_idEps" <?php echo 'value="'.$destino_idEps.'"'; ?>>
				</div>
			</div>
