<?php
/**
 * egali-sass functions and definitions
 * @link https://developer.wordpress.org/themes/basics/theme-functions/
 * @package egali-sass
 */

if ( ! function_exists( 'egali_sass_setup' ) ) :
	/**
	 * Runs before the init hook. The init hook is too late for some features, such as indicating support for post thumbnails.
	 */
	function egali_sass_setup() {
		load_theme_textdomain( 'egali-sass' );
		add_theme_support( 'title-tag' );
		add_theme_support( 'post-thumbnails' );

		// This theme uses wp_nav_menu() in one location.
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'egali-sass' ),
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'egali_sass_custom_background_args', array(
...
add_action( 'after_setup_theme', 'egali_sass_setup' );
...
function egali_enqueue_styles() {
	...
	wp_enqueue_style( 'egali-styles', get_stylesheet_directory_uri() . '/css/style-master.css' );
	...
	wp_enqueue_style( 'egali-styles', get_stylesheet_directory_uri() . '/css/style.css' );
}
 ...
 */
/**
 * Banners to the slider of the header.
 */
